...
function validateDioceseFile(file) {
  ...
  for (const p of data.parishes) {
    ...
    else if (!p.name || !/^((S[ée] )?Catedral|Par[óo]quia|(Con|Co-)catedral|Santu[áa]rio|Reitoria|(Quase|Pr[óo])-par[óo]quia|Bas[íi]lica|Curato|Rede de Comunidades|[ÁA]rea (Pastoral|Mission[áa]ria)|(Unidade|Regi[ãa]o) Pastoral)/i.test(p.name)) warn(file, `nome sem prefixo padronizado: ${p.name}`);
