...
export class TitheCampaignsService {
  ...
  private present(c: CampaignRow, t?: Totals) {
    ...
    const percent = c.goalAmount && c.goalAmount > 0 ? Math.min(100, Math.round((raised / c.goalAmount) * 1000) / 10) : null;
    const daysLeft = c.endsAt ? Math.max(0, Math.ceil((c.endsAt.getTime() - Date.now()) / 86400000)) : null;
  ...
  private normalize(dto: {
    ...
    if (dto.name !== undefined) {
      const name = String(dto.name ?? '').replace(/[\r\n\t]+/g, ' ').trim().slice(0, 80);
      if (name.length < 3) throw new BadRequestException('Dê um nome à campanha (pelo menos 3 letras)');
      out.name = name;
    }
    if (dto.description !== undefined) out.description = (dto.description ?? '').trim().slice(0, 1000) || null;
    ...
    const parseDate = (value: string | null | undefined, endOfDay: boolean) => {
      if (value === null || value === undefined || value === '') return null;
      const raw = String(value).slice(0, 10);
      if (!/^\d{4}-\d{2}-\d{2}$/.test(raw)) throw new BadRequestException('Data inválida (use AAAA-MM-DD)');
      const date = new Date(`${raw}T${endOfDay ? '23:59:59.999' : '00:00:00.000'}-03:00`);
  ...
  async create(
    ...
    let parishId = dto.parishId || user.parishId || null;
    if (communityId) {
  ...
  async activate(user: CurrentUser, id: string) {
    ...
    const updated = await this.prisma.titheCampaign.update({
      where: { id },
      data: { status: 'ACTIVE', startsAt: current.startsAt ?? new Date() },
      select: this.select,
    });
    ...
    try {
      ...
      if (ids.length) {
        ...
        await this.notificationsService.notifyUsers(
          ...
          `${(updated.description ?? '').slice(0, 140)}${goal}${until} Contribua pelo app, na área do dízimo.`.trim(),
  ...
  async addEntry(
    ...
    const method = (dto.method ?? 'Dinheiro').replace(/[\r\n\t]+/g, ' ').trim().slice(0, 30) || 'Dinheiro';
    const description = (dto.description ?? '').replace(/[\r\n\t]+/g, ' ').trim().slice(0, 200);
    let communityId = dto.communityId || campaign.communityId || null;
    if (communityId) {
      const community = await this.prisma.community.findUnique({ where: { id: communityId }, select: { parishId: true } });
      ...
      const scope = await this.tithe.financeScope(user);
      if (scope.communityIds) communityId = scope.communityIds[0] ?? null;
    }
  ...
  /** Relatório: totais, split por comunidade e por meio, promessas, contribuições e lançamentos. */
  async report(user: CurrentUser, id: string) {
    ...
    const [totals, transactions, intents, pledges, communities] = await Promise.all([
      ...
      this.prisma.financialTransaction.findMany({
        ...
        orderBy: { date: 'desc' },
        take: 1000,
        select: { id: true, type: true, amount: true, date: true, description: true, communityId: true },
      }),
      this.prisma.titheIntent.findMany({
      ...
      this.prisma.titheCampaignPledge.findMany({
        where: { campaignId: id, status: 'OPEN' },
    ...
    const manualTotal = Math.round((t.raised - t.appTotal) * 100) / 100;
    if (manualTotal > 0) byMethod.set('MANUAL', { total: manualTotal, count: Math.max(0, t.count - intents.length) });
    // Cumprimento das promessas: confirmado por membro
    const givenByMember = new Map<string, number>();
    for (const i of intents) givenByMember.set(i.member.id, Math.round(((givenByMember.get(i.member.id) ?? 0) + (i.amountPaid ?? i.amount)) * 100) / 100);
    ...
    return {
      ...
      contributions: intents.map((i) => ({
        ...
        member: i.anonymous ? { id: null, fullName: 'Oferta anônima' } : { id: i.member.id, fullName: i.member.fullName },
        method: i.paymentMethod ?? 'PIX',
        community: nameOf.get(i.communityId ?? '') ?? '—',
        txid: i.txid,
      })),
      entries: transactions.map((e) => ({
        id: e.id,
        date: e.date,
  ...
  async reportCsv(user: CurrentUser, id: string): Promise<string> {
    const r = await this.report(user, id);
    const esc = (v: unknown) => `"${String(v ?? '').replace(/"/g, '""')}"`;
    const lines = [['Data', 'Origem', 'Valor', 'Meio', 'Quem', 'Comunidade', 'Descrição'].map(esc).join(';')];
    for (const c of r.contributions) {
      lines.push([day(c.date), 'App', c.amount.toFixed(2).replace('.', ','), c.method, c.member.fullName, c.community, c.txid].map(esc).join(';'));
    }
    for (const e of r.entries) {
      if (e.description?.includes('(manual') || e.type === 'EXPENSE') {
        lines.push([day(e.date), e.type === 'EXPENSE' ? 'Estorno' : 'Manual', e.amount.toFixed(2).replace('.', ','), '', '', e.community, e.description ?? ''].map(esc).join(';'));
      }
    }
    lines.push(['', 'Total', r.raised.toFixed(2).replace('.', ','), '', '', '', r.campaign.name].map(esc).join(';'));
    return '\uFEFF' + lines.join('\r\n');
  }
